...
public class KategoriController {
	...
	public ModelAndView kategori(ModelAndView mav, KategoriAttribute form){
		...
		kategoriForm.setKategories(kategoriService.findKategoriesByName("",MatchMode.ANYWHERE));
		
		if(form.getFilter()!=null){
			kategoriForm.setKategories(kategoriService.findKategoriesByName(form.getFilter(),MatchMode.ANYWHERE));
		}
	...
	public ModelAndView deleteKategori(ModelAndView mav, KategoriAttribute form){
		...
		if(form.getDeletedKategori()!=null){
			deletedKategori.setKategoriId(Double.parseDouble(form.getDeletedKategori()));
	//		bankService.deleteBank(deletedBank);
			logger.error("Kategori Deleted id:"+deletedKategori.getKategoriId());
		}
		kategoriForm.setKategories(kategoriService.findKategoriesByName("",MatchMode.ANYWHERE));
		mav.addObject("form",kategoriForm);
		mav.setViewName(KATEGORI);						
